...
def create_pulumi_program(content: str):
    # Create Bucket and expose website index
    site_bucket = s3.Bucket('s3-site-bucket', website=s3.BucketWebsiteArgs(index_document="index.html"))
    index_content = content

    # Add HTML in site bucket
    s3.BucketObject(
        "index",
...
def create_site():
    ...
    if request.method == 'POST':
        ...
        if(file_url):
            ...
            try:
                # Create a new stack, generating pulumi program from POST body
                stack = auto.create_stack(
                    stack_name=str(stack_name),
                    project_name=current_app.config["PROJECT_NAME"],
                    pulumi_program=pulumi_program
                )
                stack.set_config('aws:region', auto.ConfigValue("asia-south-1"))
                # Deploy stack
                stack.up(on_output=print)
                flash(
                    f"Successfuly created site '{stack_name}'",
                    category='success'
                )

            except auto.StackAlreadyExistsError:
                flash(
                    "Error with site name '{stack_name}' already exists, pick a unique name", 
                    category="danger"
                )
